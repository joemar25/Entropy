...
export default function Dashboard() {
    ...
    const [selectedMetrics, setSelectedMetrics] = useState<string[]>([
        'temperature', 'humidity', 'pm25', 'voc', 'o3', 'co', 'co2', 'no2', 'so2'
    ])
    ...
    useEffect(() => {
        ...
        try {
            ...
            switch (timeFilter) {
                ...
                case '1h':
                    filteredData = filteredData.filter(point =>
                        new Date(point.timestamp).getTime() >= now - 3600 * 1000
                    );
                    break;
                case '6h':
                    filteredData = filteredData.filter(point =>
                        new Date(point.timestamp).getTime() >= now - 6 * 3600 * 1000
                    );
                    break;
                case '24h':
                    filteredData = filteredData.filter(point =>
                        new Date(point.timestamp).getTime() >= now - 24 * 3600 * 1000
                    );
                    break;
                case 'all':
                default:
            ...
            setChartData(filteredData.map(point => ({
            ...
            const filteredIndices = data.timestamp.map((ts, index) =>
                new Date(ts).getTime() >= twentyFourHoursAgo ? index : -1
